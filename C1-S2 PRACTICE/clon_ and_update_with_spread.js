...
let students = [
  ...
  function updateStudentGrade(studentsArray, idToUpdate, newGrade) {
    ...
    if (indexToUpdate !== -1) {
      // Step 2 : If the student with the provided ID is found , create a copy of the original array using the spread <operator className=""></operator>

      const copyStudent = [...studentsArray];
      // Step 3 : Update the grade of the student at the found index in the copied array.
      copyStudent[indexToUpdate].grade = newGrade;
      return copyStudent;
    } else {
